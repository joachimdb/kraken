...
(take! c (fn [v] (println "Got " v) (flush)))
...
(put! c "Hello World" (fn [] (println "Done putting") (flush)))
...
(def d (atom 0))
(put! c "Hello World" (fn [] (println "Done putting") (swap! d inc)))
@d

(def g (atom 0))
(take! c (fn [v] (println "Got " v) (swap! g inc)))
@g
...
(defn takep [c]
  (let [p (promise)] ;; promisse to be delivered at some time later, blocks when @/deref'ed before that
    (take! c (fn [v] (deliver p v)))
    @p))

;; Now we can block the current thread waiting on the promise.
(def f (future (println "Got!" (takep c)))) ;;; (future f) calls f in another thread. Get value with @/deref
@f
(put! c 42)
