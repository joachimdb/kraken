...
(ticks :query (daterange-filter :time (tcore/ago (tcore/minutes 30)))
       :sort [{:time {:order "desc"}}])

;; all XLTCZEUR ticks from the last half our in descending order:
(ticks :query (match-query :asset "XLTCZEUR")
       :sort [{:time {:order "desc"}}])

;; last XLTCZEUR tick:
(ticks :query (filtered-query :query (match-query :asset "XLTCZEUR")
                              :filter (daterange-filter :time (tcore/ago (tcore/minutes 30))))
       :sort [{:time {:order "desc"}}]
       :size 1)
